...
//이전 글 링크
function the_before_board_link(){
	...
	if($results = $wpdb->get_results($sql)){
		...
		?><tr><th><?php echo __('Previous','mhboard');?></th><td colspan="5"><a href="<?php echo get_permalink($board_id);?>" title="<?php echo __('Previous','mhboard');?>"><?php echo get_the_title($board_id);?></a></td><?php
	}
	
}
function the_after_board_link(){
	global $mh_board, $wpdb,$pagename;
	$term_id = $mh_board->current_board_cat;
	$post_id = $mh_board->current_board_id;
	...
	if($results = $wpdb->get_results($sql)){
		$board_id = $results[0]->ID;
		?><tr><th><?php echo __('Next','mhboard');?></th><td colspan="5"><a href="<?php echo get_permalink($board_id);?>" title="<?php echo __('Next','mhboard');?>"><?php echo get_the_title($board_id);?></a></td><?php
	}
}
...
function get_mh_board_id(){
	global $wp_rewrite;
	if($wp_rewrite->permalink_structure == '' && isset($_GET['ID']) && is_numeric($_GET['ID'])){
		return $_GET['ID'];
	}else{
